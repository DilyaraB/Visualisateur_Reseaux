            ...
            for l in lignes:
                if (l!='\n'): #if ligne exists
                    ...
                    if (offset=="0000"):
                        trame = ""
                        if (len(trames)>999):
                            print("Entree de +1000 trames. 1000 est la limite de nombre de trames que le programme peut afficher. Nous sommes desolees.")
                            break
                        trames.append(trame)
                    bytes = parts[1].split()
...
def randomColor(nombre) :
    ...
    for j in range(nombre) :
        ...
        while c<="#AAAAAA" :
            c="#"+''.join([random.choice('ABCDEF0123456789') for i in range(6)])
        #BETA VERSION: pour eviter des couleurs similaires
        if (j > 0) :
            new=hextoRGB(c)
            prev=hextoRGB(rand_colors[j-1])
            if(colorsAreClose(new, prev)) :
                while colorsAreClose(new, prev) :
                    c="#"+''.join([random.choice('ABCDEF0123456789') for i in range(6)])
                    new=hextoRGB(c)
                    prev=hextoRGB(rand_colors[j-1])  
...
############## MAIN ###############
###################################
##creation de l'interface graphique pour la sortie et quelques fonctionnes necessaires pour tk
first=True
print("Entrez le chemin/le nom du fichier text :\n")
filename = input()
createFile(filename)

fenetre = Tk() #creation de la fenetre d'interface graphique

indexLigne=0 #pour savoir on est dans quelle ligne
indexColor=0 #pour savoir le nb de connections, chaque fois qd on trouve la nouvelle connection, on l'incremente
MSG=1

premierLigne=True #pour la premiere ligne on va creer un autre label
fenetre.title('Visualisateur des flux de trafic reseau')
fenetre.geometry("980x900")
#fenetre.attributes('-fullscreen', True) #pour afficher la fenetre fullscreen           
frame = Frame(fenetre)
frame.pack(fill=BOTH,expand=1)
c= Canvas(frame)
c.delete("all")
c.pack(side=LEFT, fill=BOTH, expand=1)
scroll = ttk.Scrollbar(frame, orient=VERTICAL, command=c.yview)
scroll.pack(side=RIGHT,fill=Y)

labelEntry=Label(fenetre, text="Pour filtrer:\n les @IP client, tapez ipc=='@IPClient' \n les @IP serveur, tapez ips=='@IPServeur' \n les HTTP, tapez HTTP ou http \n les TCP tapez TCP ou tcp")
labelEntry.pack(side=LEFT)
entry = Entry(fenetre, width=50)
entry.pack(pady=30, padx=40, side=LEFT)

c.configure(yscrollcommand=scroll.set)
c.bind('<Configure>',lambda e:c.configure(scrollregion=c.bbox("all")))

incan = Frame(c)
c.create_window((0,0), window=incan, anchor="nw")

#p.pack(side=TOP, expand=Y, fill=BOTH, pady=2, padx=2)
connections= {} #lit les trames et cree une dictionnaire avec la cle (@ipClient, @ipServeur) et la valeur est un int=le numero de connection
#labels=[] #le resultat de la sortie 
randColors=randomColor(MAXMSG) #la liste des couleurs qui sont generes aleatoirement

fichier = open(createFile(filename),"r")
lignes = fichier.readlines() #liste contenant les lignes du fichier
fichier.close()

def affichageLabel(couple,protoLigne,l):
    global indexColor
    #on cree un label avec la connection existante si on a deja cette connection sinon on cree une nouvelle connection
    if(couple in connections) : 
        couleur=connections[couple]
        valeur='\t'+protoLigne+l
        Label(incan, text=valeur, bg=couleur, fg="black",justify=LEFT).grid()
    else :
        couleur=randColors[indexColor]
        indexColor+=1
        valeur='\t'+protoLigne+l
        connections[couple]=couleur
        Label(incan, text=valeur, bg=couleur, fg="black",justify=LEFT).grid()
        ...
        for l in lignes :
            if (l!='\n'):
                if(premierLigne) :
                    premierLigne=False
                    elts = l.split()
                    trueFormat = elts[0]+'\t'+elts[1]+'\t'+elts[2]+'\t\t\t'+elts[3]+'\t\t\t'+elts[4]+'\t'+elts[5]+'\t'
                    Label(incan, text=trueFormat, bg="white", fg="black",justify=LEFT).grid()
                    continue
                ...
                if (MSG%2==0):
                    ...
                    portServeur=coupe[6]
                    ipServeur=coupe[7]
                    protoLigne=lignes[indexLigne]
                    couple=(ipClient, ipServeur, portClient, portServeur)

                    affichageLabel(couple,protoLigne,l)
                MSG+=1
            indexLigne+=1
        ...
        if (l!='\n'):
            ...
            if (MSG%2==0):
                ...
                if ((entry.get())==""):
                    ...
                    affichageLabel(couple,protoLigne,l)
                if ("ipc==" in (entry.get()) and "ips==" in (entry.get())):
                    if ("ipc=="+ipClient in (entry.get()) and "ips=="+ipServeur in (entry.get())):
                        if ("HTTP" in entry.get() or ("http" in entry.get())):
                            if ("HTTP" in protoLigne):
                                affichageLabel(couple,protoLigne,l)
                        elif (("TCP" in entry.get()) or "tcp" in entry.get()):
                            if ("TCP" in protoLigne):
                                affichageLabel(couple,protoLigne,l)
                        else:
                            affichageLabel(couple,protoLigne,l)
                elif ("ipc==" in (entry.get())):
                    if ("ipc=="+ipClient in (entry.get())) :
                        if ("HTTP" in entry.get() or ("http" in entry.get())):
                            if ("HTTP" in protoLigne):
                                affichageLabel(couple,protoLigne,l)
                        elif (("TCP" in entry.get()) or "tcp" in entry.get()):
                            ...
                            affichageLabel(couple,protoLigne,l)
                elif ("ips==" in (entry.get())):
                    if ("ips=="+ipServeur in (entry.get())) :
                        if ("HTTP" in entry.get() or ("http" in entry.get())):
                            if ("HTTP" in protoLigne):
                                affichageLabel(couple,protoLigne,l)
                        elif (("TCP" in entry.get()) or "tcp" in entry.get()):
            ...
            MSG+=1
            #else :
                #print("Le nombre de trame qu'on peut afficher est "+str(MAXMSG))
                #break
        indexLigne+=1
...
button=Button(fenetre, text="Filter",command=filtering)
button.pack(side=LEFT, padx=3)
resetButton=Button(fenetre,text="Reset",command=reset)
resetButton.pack(padx=15, side=LEFT)
c.pack()
